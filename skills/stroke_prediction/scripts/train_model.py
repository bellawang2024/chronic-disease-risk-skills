...
# -*- coding: utf-8 -*-
"""
脑卒中（Stroke）风险预测模型 - 训练与验证脚本
================================================================
数据来源: BRFSS 2017 (CDC Behavioral Risk Factor Surveillance System)
          美国 50 州 + 属地的全人群电话调查抽样 —— 全人群, 不限性别/种族
样本:     448,666 名成人 (>=18 岁)
特征:     15 个简洁的风险因素指标 (问卷 + 派生)
标签:     CVDSTRK3 — "曾被医生告知患有脑卒中"
模型:     XGBoost 二分类

重要: 本任务阳性率仅约 4.2%，**准确率(accuracy)不是有效指标**
      (多数类基线即有 95.78%)。评估以 AUC + 平衡准确率 + 敏感度/特异度为准，
...
DEFAULT_DATA = os.path.join(DATA_DIR, "stroke_brfss.csv")
...
# 说明: 派生数据集在构建阶段就完全未提取 SEX / _RACE，也未提取
# 冠心病/心梗结局变量(_MICHD)，因此模型在结构上不使用人群划分属性。
TARGET = "label"
...
def check_acceptance(holdout, cv, verbose=True):
    ...
    if verbose:
        ...
        print("\n----- 验收标准检查 -----")
        print("  （本任务阳性率仅约 4.2%，故不用普通准确率作为验收指标）")
...
def train_model(data_path=None, quick=False, verbose=True):
    ...
    model_json = os.path.join(MODELS_DIR, "stroke_xgb_model.json")
    model_ubj = os.path.join(MODELS_DIR, "stroke_xgb_model.ubj")
    ...
    metadata = {
        ...
        "dataset": {
            ...
            "cohort_definition": "≥18 岁且有脑卒中作答记录的受访者",
            "file": "data/stroke_brfss.csv",
            ...
            "target": TARGET,
            "target_definition": "BRFSS 变量 CVDSTRK3 = 曾被医生告知患有脑卒中（1=是 2=否）",
            "target_labels": {"0": "无脑卒中", "1": "有脑卒中"},
        ...
        "decision_threshold_note": (
            ...
            "该任务阳性率仅约 4.2%，固定 0.5 会导致敏感度约 0（模型完全不可用）"
        ),
        "validation": {
            "holdout": holdout,
            "cross_validation": cv,
        ...
        "artifacts": {
            "model_json": "stroke_xgb_model.json",
            "model_ubj": "stroke_xgb_model.ubj",
...
def main():
    parser = argparse.ArgumentParser(description="脑卒中风险预测模型 - 训练与验证")
