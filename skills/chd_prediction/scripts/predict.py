...
# -*- coding: utf-8 -*-
"""
冠心病（CHD）风险预测 - 单个体推理脚本
================================================================
加载 models/ 下的推理工件，对个体做冠心病/心梗风险二分类预测。

预处理与训练一致：缺失特征用训练中位数插补，XGBoost 输出概率，
再用**训练集交叉验证选定的阈值**（默认 0.08，不是 0.5）判定。

注意: 本任务阳性率仅约 8.8%，概率数值本身偏小属正常现象。
      风险等级对照的是实测患病率，不是 30%/70% 这种直觉分档。

用法:
    # 6 项主要临床指标即可预测
    python scripts/predict.py --age 62 --bmi 29.5 --smoker 3 --diabetes 0 \
        --hypertension 0 --high-chol 1

    # 补充更多指标（可选）
    python scripts/predict.py --age 62 --bmi 29.5 --smoker 3 --diabetes 0 \
        --hypertension 0 --high-chol 1 --gen-health 3 --copd 0 --exercise 1 --json

作为模块调用:
    from predict import predict_patient
    predict_patient(age=62, bmi=29.5, smoker=3, diabetes=0, hypertension=0, high_chol=1)
================================================================
"""
import argparse
import json
...
# 风险分档：括号内为留出测试集上该档的实测冠心病患病率（模型校准良好）
RISK_BANDS = [
    (0.08, "低风险", "实测患病率约 1%–6%；保持健康生活方式，定期体检"),
    (0.15, "中等风险", "实测患病率约 11%；建议控制血压血脂、体重与饮食，定期复查"),
    (0.30, "较高风险", "实测患病率约 22%；建议尽快就医，评估血压、血脂、血糖与心电图"),
    (1.01, "高风险", "实测患病率约 42%；强烈建议尽快就医，进行心血管专科评估"),
]
...
def _load_model():
    ...
    for name in ("chd_xgb_model.json", "chd_xgb_model.ubj"):
...
def predict_patient(model=None, medians=None, order=None, metadata=None, **features):
    ...
    threshold = float(metadata.get("decision_threshold", 0.08))
    ...
    return {
        ...
        "label": "冠心病/心梗阳性" if prediction == 1 else "冠心病/心梗阴性",
        "risk_level": level,
        "risk_note": note,
...
def _print_report(result, sources):
    print("\n" + "=" * 66)
    print("            冠心病（CHD）风险预测结果")
    print("=" * 66)
    print(f"\n风险概率: {result['probability_pct']}   (判定阈值 {result['decision_threshold']})")
    print(f"预测结果: {'阳性 - 提示存在冠心病/心梗' if result['prediction'] == 1 else '阴性 - 未提示冠心病/心梗'}")
...
def main():
    p = argparse.ArgumentParser(description="冠心病（CHD）风险预测（单个体）")
