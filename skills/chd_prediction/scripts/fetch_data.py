...
冠心病建模队列 data/coronary_heart_disease_brfss.csv。
...
注意:
  ...
  2. 原始 XPT 解压后约 1.29 GB，解析约需 50 秒与约 2 GB 内存。
...
OUT_CSV = os.path.join(DATA_DIR, "coronary_heart_disease_brfss.csv")
...
EXPECTED_CSV_SHA256 = "a43af90ee0f846f6e0ba72e47c1068a2a6deaf9089e406c0359e2af6bb6c8055"
EXPECTED_N_SAMPLES = 445872
...
def build_cohort(xpt_path):
    ...
    out = pd.DataFrame({
        ...
        "label":        d._MICHD.map({1.0: 1.0, 2.0: 0.0}),
    })
...
def main():
    parser = argparse.ArgumentParser(description="下载并构建 BRFSS 冠心病数据集")
