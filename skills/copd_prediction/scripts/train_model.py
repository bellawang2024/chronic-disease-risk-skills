...
# -*- coding: utf-8 -*-
"""
慢性阻塞性肺疾病（COPD）风险预测模型 - 训练与验证脚本
================================================================
数据来源: BRFSS 2017 (CDC Behavioral Risk Factor Surveillance System)
          美国 50 州 + 属地的全人群电话调查抽样 —— 全人群, 不限性别/种族
样本:     447,718 名成人 (>=18 岁)
特征:     14 个简洁的风险因素指标 (问卷 + 派生)
标签:     CHCCOPD1 — "曾被医生告知患有慢阻肺(COPD)、肺气肿或慢性支气管炎"
模型:     XGBoost 二分类

重要: 本任务阳性率仅约 8.4%，**准确率(accuracy)不是有效指标**
      (多数类基线即有 91.61%)。评估以 AUC + 平衡准确率 + 敏感度/特异度为准，
...
DEFAULT_DATA = os.path.join(DATA_DIR, "copd_brfss.csv")

# ---------------------------------------------------------------------------
# 特征定义
# ---------------------------------------------------------------------------
# 说明:
#   1. 派生数据集在构建阶段就完全未提取 SEX / _RACE，模型结构上不使用人群划分属性。
#   2. CHCCOPD1 是标签；COPDCOGH/COPDFLEM/COPDBRTH/COPDBTST/COPDSMOK 是仅对 COPD
#      患者追问的后续问题（标签泄漏），全部未使用。
#   3. ASTHMA3（哮喘）属同类呼吸系统疾病，同样排除以保持"纯风险因素"语义。
ALL_FEATURES = [
...
FEATURE_RANGES = {
    ...
    "drinks_wk": (0, 200), "depression": (0, 1),
}
...
def check_acceptance(holdout, cv, verbose=True):
    ...
    if verbose:
        ...
        print("\n----- 验收标准检查 -----")
        print("  （本任务阳性率仅约 8.4%，故不用普通准确率作为验收指标）")
...
def train_model(data_path=None, quick=False, verbose=True):
    ...
    model_json = os.path.join(MODELS_DIR, "copd_xgb_model.json")
    model_ubj = os.path.join(MODELS_DIR, "copd_xgb_model.ubj")
    ...
    metadata = {
        ...
        "dataset": {
            ...
            "cohort_definition": "≥18 岁且有 COPD 作答记录的受访者",
            "file": "data/copd_brfss.csv",
            ...
            "excluded_for_leakage_note": "上述变量为仅对 COPD 患者追问的后续问题；ASTHMA3(哮喘) 亦排除",
            "target": TARGET,
            "target_definition": "BRFSS 变量 CHCCOPD1 = 曾被医生告知患有慢阻肺(COPD)、肺气肿或慢性支气管炎（1=是 2=否）",
            "target_labels": {"0": "无 COPD", "1": "有 COPD"},
        ...
        "decision_threshold_note": (
            ...
            "该任务阳性率仅约 8.4%，固定 0.5 会导致敏感度仅约 0.11"
        ),
        "validation": {
            "holdout": holdout,
            "cross_validation": cv,
        ...
        "artifacts": {
            "model_json": "copd_xgb_model.json",
            "model_ubj": "copd_xgb_model.ubj",
...
def main():
    parser = argparse.ArgumentParser(description="COPD 风险预测模型 - 训练与验证")
