...
def load_importance():
    ...
    model_path = os.path.join(MODELS_DIR, "copd_xgb_model.json")
...
def main():
    parser = argparse.ArgumentParser(description="查看 COPD 模型特征重要性")
    ...
    print("=" * 78)
    print("说明: 吸烟状态排名靠前，与 COPD 的病因学一致（吸烟是最主要的可干预危险因素）。")
    print("      但自评健康状况排在首位，其重要性主要来自\"已确诊者自评更差\"的反向因果，")
    print("      而年龄的排序低于其真实流行病学地位（年龄由 2% 升至 13% 的患病率差异），")
    print("      因为其效应部分被吸烟状态等中介变量吸收。解读时需注意这三点。")
    ...
    if args.plot:
        ...
        ax.bar(labels, values, color="#2980B9")
        ax.set_ylabel("importance")
        ax.set_title("COPD Risk - Feature Importance (XGBoost)")
