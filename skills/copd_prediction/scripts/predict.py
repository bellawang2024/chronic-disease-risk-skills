...
# -*- coding: utf-8 -*-
"""
慢性阻塞性肺疾病（COPD）风险预测 - 单个体推理脚本
================================================================
加载 models/ 下的推理工件，对个体做 COPD 风险二分类预测。

预处理与训练一致：缺失特征用训练中位数插补，XGBoost 输出概率，
再用**训练集交叉验证选定的阈值**（默认 0.09，不是 0.5）判定。

注意: 本任务阳性率仅约 8.4%，概率数值本身偏小属正常现象。
...
用法:
    ...
    # 补充更多指标（可选）
    python scripts/predict.py --age 62 --bmi 26.5 --smoker 1 --diabetes 0 \
        --hypertension 1 --high-chol 0 --gen-health 3 --exercise 0 --json

作为模块调用:
    from predict import predict_patient
    predict_patient(age=62, bmi=26.5, smoker=1, diabetes=0, hypertension=1, high_chol=0)
================================================================
"""
import argparse
import json
...
# 风险分档：括号内为留出测试集上该档的实测 COPD 患病率（模型校准良好）
RISK_BANDS = [
    (0.04, "低风险", "实测患病率约 1%–3%；保持健康生活方式，避免吸烟与二手烟"),
    (0.09, "中等风险", "实测患病率约 6%；建议戒烟、定期肺功能检查（尤其有咳嗽咳痰症状时）"),
    (0.15, "较高风险", "实测患病率约 11%；建议尽快就医，进行肺功能（支气管舒张试验）检查"),
    (0.30, "高风险", "实测患病率约 21%；建议进行呼吸专科评估与肺功能检查，并戒烟干预"),
    (1.01, "极高风险", "实测患病率约 43%；强烈建议尽快就医做呼吸专科评估与肺功能检查"),
]
...
def _load_model():
    ...
    for name in ("copd_xgb_model.json", "copd_xgb_model.ubj"):
...
def predict_patient(model=None, medians=None, order=None, metadata=None, **features):
    ...
    threshold = float(metadata.get("decision_threshold", 0.09))
    ...
    return {
        ...
        "label": "COPD 阳性" if prediction == 1 else "COPD 阴性",
        "risk_level": level,
        "risk_note": note,
...
def _print_report(result, sources):
    print("\n" + "=" * 68)
    print("        慢性阻塞性肺疾病（COPD）风险预测结果")
    print("=" * 68)
    print(f"\n风险概率: {result['probability_pct']}   (判定阈值 {result['decision_threshold']})")
    print(f"预测结果: {'阳性 - 提示存在 COPD' if result['prediction'] == 1 else '阴性 - 未提示 COPD'}")
    ...
    print("      不能替代专业医生的诊断与肺功能检查。")


def main():
    p = argparse.ArgumentParser(description="COPD 风险预测（单个体）")
    ...
    try:
        result = predict_patient(
            ...
            drinks_wk=args.drinks_wk, depression=args.depression,
        )
