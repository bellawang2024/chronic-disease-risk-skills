...
OUT_CSV = os.path.join(DATA_DIR, "copd_brfss.csv")
...
EXPECTED_CSV_SHA256 = "386ab988726a6ad6047bbd0d8858058293a6ed5b546ff70430b87fbf275c65d7"
EXPECTED_N_SAMPLES = 447718
EXPECTED_N_RAW = 450016

OUTPUT_COLUMNS = [
    "age", "bmi", "height_cm", "weight_kg", "smoker", "diabetes",
    "hypertension", "high_chol", "gen_health", "exercise", "education",
    "income", "drinks_wk", "depression", "label",
]
...
def build_cohort(xpt_path):
    ...
    out = pd.DataFrame({
        ...
        "label":        d.CHCCOPD1.map({1.0: 1.0, 2.0: 0.0}),
    })
...
def main():
    parser = argparse.ArgumentParser(description="下载并构建 BRFSS COPD 数据集")
