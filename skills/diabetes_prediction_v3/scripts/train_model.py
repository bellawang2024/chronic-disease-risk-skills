...
# -*- coding: utf-8 -*-
"""
糖尿病风险预测模型（v3）- 训练与验证脚本
================================================================
数据来源: NHIS 2016-2025 (CDC/NCHS National Health Interview Survey)
          美国非机构化平民人口全人群住户抽样 —— **男女兼备、全族裔**
样本:     291,945 名成人 (>=18 岁)
特征:     7 项**风险因素**（不含任何诊断性生化指标）
标签:     2018 及以前 DIBEV1；2019 及以后 DIBEV_A
          → "曾被医生告知患有糖尿病"
模型:     XGBoost 二分类

为什么用 NHIS 而不是 Pima（v2）:
  Pima 数据集有**三重人群限制**——官方原始文件写明
  "all patients here are females at least 21 years old of Pima Indian heritage"，
  即仅女性、≥21 岁、仅 Pima 印第安人，共 768 例，且数据中根本没有性别列。
  NHIS 为全人群抽样（男 143,524 / 女 158,681），阳性 31,760 例，
  是 Pima 的 118 倍。

为什么只用风险因素、不纳入生化指标:
  1. NHIS 本身不含生化检验数据（天然满足该要求）。
  2. 更重要的是**逻辑**：HbA1c ≥6.5% 与空腹血糖 ≥126 mg/dL **本身就是糖尿病的
     诊断标准**，用它们预测糖尿病近乎循环论证。因此本模型只使用
     年龄、体格、生活方式与代谢综合征组分等**风险因素**。

重要: 本任务阳性率约 10.9%，多数类基线准确率即有 89.12%，
      **普通准确率参考价值有限**。评估以 AUC + PR-AUC + 平衡准确率 +
      敏感度/特异度为准，决策阈值由训练集交叉验证选出，而非固定 0.5。
================================================================
用法:
...
DEFAULT_DATA = os.path.join(DATA_DIR, "diabetes_nhis.csv")

# ---------------------------------------------------------------------------
# 特征定义：全部为**风险因素**，无任何诊断性生化指标
# ---------------------------------------------------------------------------
# 选择依据（实测，见 DATA_PROVENANCE）：
#   仅核心风险因素(年龄/BMI/身高/体重/吸烟)          → CV AUC 0.7677
#   + 代谢综合征组分(高血压/高胆固醇)  ← 本模型采用    → CV AUC 0.8123
#   + 冠心病/脑卒中/慢阻肺（糖尿病并发症，反向因果）    → CV AUC 0.8162（仅 +0.004）
#
# 高血压与高胆固醇是代谢综合征组分，为标准糖尿病风险评分（如 FINDRISC）所包含，
# 加入带来 +0.045 AUC 的实质提升；
# 而冠心病/脑卒中/慢阻肺是糖尿病的**并发症**（反向因果明显），仅 +0.004，
# 故排除以保持"风险因素 → 疾病"的因果方向清晰。
...
THRESHOLD_GRID = (0.02, 0.90, 0.005)

# 风险分档边界（由留出测试集实测患病率确定，见 SKILL.md 的分档表）
RISK_BAND_EDGES = [[0.06, "低风险"], [0.12, "人群基线"], [0.22, "中等风险"], [0.40, "较高风险"]]
RISK_BAND_TOP = "高风险"
RISK_BAND_CALIBRATION = {
    "<0.06": {"人群占比": 0.498, "实测糖尿病率": 0.0227, "相对基线": 0.21},
    "0.06-0.12": {"人群占比": 0.168, "实测糖尿病率": 0.0831, "相对基线": 0.76},
    "0.12-0.22": {"人群占比": 0.157, "实测糖尿病率": 0.1660, "相对基线": 1.53},
    "0.22-0.40": {"人群占比": 0.144, "实测糖尿病率": 0.2953, "相对基线": 2.71},
    ">=0.40": {"人群占比": 0.034, "实测糖尿病率": 0.4458, "相对基线": 4.10},
}
...
# 验收标准：与同系列慢病 skill 保持同一标准（AUC>=0.80、平衡准确率>=0.70）
ACCEPTANCE = {
    "holdout_auc": 0.80,
...
class NamedXGB(xgb.XGBClassifier):
    """在 fit 时把 numpy 数组还原为带列名的 DataFrame，使模型工件保留真实特征名。"""
...
def binary_metrics(y_true, proba, threshold):
    ...
    return {
        ...
        "pr_auc_lift": round(float(average_precision_score(y_true, proba) / np.mean(y_true)), 2),
...
def check_acceptance(holdout, cv, verbose=True):
    ...
    if verbose:
        ...
        print("\n----- 验收标准检查 -----")
        print("  （阳性率 10.9%，验收线与同系列慢病 skill 一致）")
...
def train_model(data_path=None, quick=False, verbose=True):
    ...
    if verbose:
        n_pos = int(y.sum())
        print("=" * 74)
        print(" 糖尿病风险预测模型 v3（全人群，仅风险因素）- 训练与验证")
        print("=" * 74)
    ...
    if verbose:
        ...
        for _, r in importance.iterrows():
            print(f"  {r['feature']:14s} ({r['feature_cn'][:22]}): {r['importance']:.4f}")

    medians = {f: round(float(v), 4) for f, v in zip(features, imputer.statistics_)}
    n_pos = int(y.sum())

    metadata = {
        "skill_name": "diabetes_prediction_v3",
        "version": "3.0.0",
        ...
        "dataset": {
            ...
            "cohort_definition": "≥18 岁且有糖尿病作答记录者",
            "supersedes": "v1/v2 使用 Pima Indians Diabetes Database（768 例，仅 ≥21 岁 Pima 印第安女性），"
                          "人群限制严重且样本极小；v3 改用全人群 NHIS",
            "file": "data/diabetes_nhis.csv",
            ...
            "no_biomarkers": "本模型**不含任何诊断性生化指标**（无 HbA1c、无血糖）。"
                             "理由：HbA1c≥6.5% 与空腹血糖≥126 mg/dL 本身即糖尿病的诊断标准，"
                             "用其预测糖尿病近乎循环论证；NHIS 数据本身也不含生化检验。",
            "excluded_complications": "冠心病/脑卒中/慢阻肺未纳入（糖尿病并发症，反向因果明显），"
                                      "实测仅贡献 +0.004 AUC",
            "target": TARGET,
            "target_definition": "2018 及以前用 DIBEV1；2019 及以后用 DIBEV_A。"
                                 "1=是 → 阳性；2=否 / 3=临界(糖尿病前期) → 阴性；7/9 → 缺失",
            "target_labels": {"0": "无糖尿病", "1": "有糖尿病"},
        ...
        "preprocessing": {
            ...
            "sentinel_cleaning": "NHIS 特殊码已置为缺失：身高 96/98/99 英寸、体重 996-998 磅",
        ...
        "risk_band_calibration": RISK_BAND_CALIBRATION,
...
def main():
    parser = argparse.ArgumentParser(description="糖尿病风险预测模型 v3 - 训练与验证")
